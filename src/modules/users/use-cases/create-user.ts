  ...
  implements UseCase<CreateUserParams, CreateUserResponse>
{
  ...
  private validateParams({
    ...
    if (hasError) {
      return new Left(
        new UserUseCaseError(UserUseCaseErrors.invalidUserProps(errors))
      );
    }
  ...
  public async execute(params: CreateUserParams): Promise<CreateUserResponse> {
    ...
    if (user.isLeft()) {
      return new Left(
        new UserUseCaseError(UserUseCaseErrors.invalidUserProps(params))
      );
    }
    ...
    if (newUserFromDb.isLeft()) {
      if (newUserFromDb.value.message.includes("duplicated")) {
        return new Left(
          new UserUseCaseError(UserUseCaseErrors.duplicatedItem(name))
        );
      }

      return new Left(
        new UserUseCaseError(UserUseCaseErrors.couldNotCreateUser)
      );
    }
